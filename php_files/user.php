<?php
    ...
    if (!isset($_POST['username']) || empty($_POST['username'])) {
        echo json_encode(array('error' => 'Username Field is Empty.'));
        exit;
        ...
        // Debugging: Log the escaped and hashed values
        error_log("Debug: Escaped Username - " . $username);
        error_log("Debug: Hashed Password - " . $password);

        $db->select('user', 'user_id,username,f_name,l_name', null, "username = '{$username}' AND password = '{$password}'", null, null);
        $response = $db->getResult();

        // Debugging: Log the response from the database
        error_log("Debug: DB Response - " . print_r($response, true));
        ...
        if (!empty($response)) {
            if (count($response[0]) > 1) {
                ...
                echo json_encode(array('error' => 'Username and Password not matched.'));
                exit;
            }
        } else {
            echo json_encode(array('error' => 'Username and Password not matched.'));
